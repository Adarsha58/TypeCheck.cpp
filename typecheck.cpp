...
void TypeCheck::visitClassNode(ClassNode *node)
{
    ...
    {
        if (!clsinfo.members->empty())
            typeError(main_class_members_present);
        if (clsinfo.methods->find("main") == clsinfo.methods->end())
            typeError(no_main_method);
        if (clsinfo.methods->at("main").parameters || clsinfo.methods->at("main").returnType.baseType != bt_none)
...
void TypeCheck::visitMethodNode(MethodNode *node)
{
    ...
    if (node->parameter_list)
    {
        ...
        {
            mi.parameters->push_back({(*prmList)->type->basetype, (*prmList)->type->objectClassName});
            mi.localsSize -= 4;
        }
    }
...
VariableInfo *validateVariable(VariableTable *varTable, ClassTable *classTable, std::string currentClassName, std::string var)
{
    ...
    if (varTable && varTable->find(var) != varTable->end())
        return &varTable->at(var);

    //checking if var is a member of this class or superclass
    while (currentClassName != "")
    {
        clsMembers = (*classTable)[currentClassName].members;
        if (clsMembers && clsMembers->find(var) != clsMembers->end())
            return &clsMembers->at(var);
...
void TypeCheck::visitAssignmentNode(AssignmentNode *node)
{
    ...
    { //case 2: t_id.t_id = expression
        if (var1Info->type.baseType != bt_object)
            typeError(not_object); //var1 has to be of classType
...
MethodInfo *validateMethod(ClassTable *classTable, std::string currentClassName, std::string metName)
{
    ...
    while (currentClassName != "")
    {
        ...
        if (clsMethods && clsMethods->find(metName) != clsMethods->end())
            return &clsMethods->at(metName);
        currentClassName = (*classTable)[currentClassName].superClassName;
    }
...
void TypeCheck::visitMethodCallNode(MethodCallNode *node)
{
    ...
    { //case 2
        ...
        if (varInfo1->type.baseType != bt_object)
...
void TypeCheck::visitMemberAccessNode(MemberAccessNode *node)
{
    ...
    if (idInfo1->type.baseType != bt_object)
        typeError(not_object);
...
void TypeCheck::visitNewNode(NewNode *node)
{
    ...
    { //second case
        ...
        { // cant find any parametrized constructor
            typeError(argument_number_mismatch);
        }
    ...
    { //first case
        ...
        { //can find the method but is parmetrized
            typeError(argument_number_mismatch);
        }
    }
